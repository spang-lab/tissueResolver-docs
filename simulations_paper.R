...
pct_genes_to_change <- c(2,5,10,15,20,30,40,50,60,70,80,90,95)
...
simulate_bulks <- function(scdata, nbulks, nperbulk, fuzzy = NA, fuzzyness = 0.3) {
  ...
  bulks <- bulkcompo %>% inner_join(scdata) %>% gather(gene, expression, -id, -bulkid, -weight) %>%
    mutate(expression = expression * weight) %>% select(-weight) %>% # multiply by weight
    group_by(bulkid, gene) %>% summarize(expression = sum(expression)/nperbulk) %>% # sum over each simulaeted bulk
...
scpheno <- as_tibble(sc$sc.pheno) %>% dplyr::select(celltype, colnames, Patient) %>% rename(id = colnames)
...
patients <- scpheno %>% pull(Patient) %>% unique()
trainpatients <- patients[sample(trainfrac*length(patients), replace=FALSE)]
scpheno <- scpheno %>% mutate(testtrain = ifelse(Patient %in% trainpatients, "train", "test")) %>% mutate(testtrain = as_factor(testtrain))

# now select cells to change:
all_these_cells <- scpheno %>% filter(celltype == celltype_to_split) %>% pull(id)
...
sclong <- sccounts %>% pivot_longer(starts_with("ENSG"), names_to="gene", values_to = "expression") %>%
  inner_join(scpheno)

n_cells_splittype <- scpheno %>% filter(celltype == celltype_to_split) %>% nrow()
n_cells <- scpheno %>% nrow()
sclong %>% filter(celltype == celltype_to_split) %>% group_by(gene) %>% mutate(zerofreq = sum(expression == 0)/n_cells_splittype)
suitable_genes <- sclong %>%
  filter(celltype == celltype_to_split) %>% group_by(gene) %>%
...
fit_bulks_tr <- function(bulks, sccounts) {
    scmat <- sccounts %>% select(starts_with("ENSG")) %>% as.matrix()
    rownames(scmat) <- sccounts %>% pull(id)
    bmat <- bulks$bulks %>% select(-bulkid) %>% as.matrix()
    ...
    return(tissueResolver::specific_expression_regulation(tissuemodel, t(scmat), sccounts %>% select(id, celltype) %>% rename(cell_id = id)))
}
...
compute_subclustering <- function(train_counts) {
	...
	for (ctype in levels(factor(train_counts %>% pull(celltype)))){
		exprmat <- train_counts %>% filter(celltype == ctype) %>% select(starts_with("ENSG")) %>% as.matrix()
		rownames(exprmat) <- train_counts %>% filter(celltype == ctype) %>% pull(id)
		...
		clusters <- seurobj@meta.data %>% rownames_to_column %>% as_tibble()
...
fit_bulks_bayesPrism <- function(bulks, sccounts, clustering) {
	bk.dat <- bulks %>% select(-bulkid, -mut) %>% as.matrix()
	rownames(bk.dat) <- bulks %>% pull(bulkid)
	sc.dat <- sccounts %>% select(starts_with("ENSG")) %>% as.matrix()
...
benchmark_tr <- function(train_counts, bulks_mut, bulks_nonmut, logfc, ngenes) {
      cse_all <- tibble()
      # fit the bulks:
      ...
      for(etype in c("expression", "regulation")){
        ...
        for(ctype in intersect(cse_mut %>% pull(celltype) %>% unique(), cse_nonmut %>% pull(celltype) %>% unique())) {
          thiscse <- cse %>% filter(celltype == ctype) %>% select(bulk_id, mut, gene, all_of(etype)) %>% pivot_wider(names_from="gene", values_from=etype, values_fill = 0.0) %>% select(mut, starts_with("ENSG"))
          sampleclasses <- thiscse %>% pull(mut) %>% as.factor()
	  csemat <- thiscse %>% select(-mut) %>% as.matrix()
          if( nrow(csemat) > 2 ) { # dispersion cannot be fitted from only two bulks.
            ...
            computed_genelist <- fit$table %>% mutate(gene = rownames(fit$table)) %>% as_tibble()
            computed_genelist <- computed_genelist %>%
              mutate(score = 1-PValue) %>% # add score col (is there sth smarter than 1-pvalue?)
              mutate(mutated = gene %in% genelist)
            precrec_obj <- evalmod(scores = computed_genelist$score, labels=computed_genelist$mutated)
            pdf(paste0("plots/roc_cse_tr_", etype, "_sim", suffix, "_", ngenes, "genes_", logfc, "lfc_", ctype, ".pdf"))
            plot(precrec_obj)
            dev.off()
...
benchmark_bayesPrism <- function(train_counts, bulks_mut, bulks_nonmut, clustering, logfc, ngenes, name) {
      cse_all <- tibble()
	...
	}, error=function(e){ edgerobj<- NULL})
	if(is.null( edgerobj )){
		   next
	}
        computed_genelist <- fit$table %>% mutate(gene = rownames(fit$table)) %>% as_tibble()
        computed_genelist <- computed_genelist %>%
          ...
          mutate(mutated = gene %in% genelist)
        precrec_obj <- evalmod(scores = computed_genelist$score, labels=computed_genelist$mutated)
        pdf(paste0("plots/roc_cse_bp_", name, "_sim", suffix, "_", ngenes, "genes_", logfc, "lfc_", ctype, ".pdf"))
        plot(precrec_obj)
        dev.off()
        aucprc <- attr(precrec_obj, "aucs")
...
for(pctchange in pct_genes_to_change ){
  ...
  for( logfc in log2foldchanges ){
    for( irun in 1:nruns ) {
      ...
      train_counts            <- mutsc %>% filter(testtrain == "train") %>% select(id, celltype, expression, gene) %>% pivot_wider(names_from="gene", values_from="expression")
      test_and_mod_counts     <- mutsc %>% filter(testtrain == "test") %>% filter((celltype != celltype_to_split) | is_selected) %>%  select(id, celltype, expression, gene) %>% pivot_wider(names_from="gene", values_from="expression")
      test_and_nonmod_counts  <- mutsc %>% filter(testtrain == "test") %>% filter((celltype != celltype_to_split) | !is_selected) %>% select(id, celltype, expression, gene) %>% pivot_wider(names_from="gene", values_from="expression")

      # now, simulate bulks:
      bulks_mut <- simulate_bulks(test_and_mod_counts %>% select(id, starts_with("ENSG")), nbulks, nperbulk, fuzzyness = 0.3)
      bulks_nonmut <- simulate_bulks(test_and_nonmod_counts %>% select(id, starts_with("ENSG")), nbulks, nperbulk, fuzzyness = 0.3)

      if( "bp_nosubtypes" %in% algos ){
        cse_all <- rbind(cse_all, benchmark_bayesPrism(train_counts, bulks_mut, bulks_nonmut, NULL, logfc, ngenes, "nosub") %>% add_column(run=irun, logfc = logfc, ngenes = ngenes, pct_genes = ngenes / length(suitable_genes)*100))
      }
      if( "bp_subtypes" %in% algos ){
        message("computing clustering")
	clustering <- compute_subclustering(train_counts)
        message("fitting")
        cse_all <- rbind(cse_all, benchmark_bayesPrism(train_counts, bulks_mut, bulks_nonmut, clustering, logfc, ngenes, "sub") %>% add_column(run=irun, logfc = logfc, ngenes = ngenes, pct_genes = ngenes / length(suitable_genes)*100))
      }
      if( "tr" %in% algos ){
        cse_all <- rbind(cse_all, benchmark_tr(train_counts, bulks_mut, bulks_nonmut, logfc, ngenes) %>% add_column(run=irun, logfc = logfc, ngenes = ngenes, pct_genes = ngenes / length(suitable_genes)*100))
      }
    }
    saveRDS(cse_all, file=paste0("data/sim_roc", suffix, ".rds.bak"))
  }
}
saveRDS(cse_all, file=paste0("data/sim_roc", suffix, ".rds"))
unlink(paste0("data/sim_roc", suffix, ".rds.bak"))
